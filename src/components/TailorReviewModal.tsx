...
export function TailorReviewModal({ proposal, onApply, onCancel }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="max-h-[85vh] w-full max-w-2xl overflow-auto rounded-xl bg-white p-6 shadow-xl dark:bg-gray-900">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          ...
          {proposal.used_ollama ? " · Ollama" : " · Rule-based"}
        </p>
            ...
            <li key={c.id} className="rounded-lg border border-gray-200 p-3 dark:border-gray-700">
              <label className="flex items-start gap-2">
                ...
                <div className="flex-1 text-sm">
                  ...
                  <p className="mt-1 line-through text-red-600/80">{c.before}</p>
        ...
        {proposal.changes.length === 0 && (
          <p className="mt-4 text-sm text-gray-500">No changes proposed — try adding more job keywords to your CV.</p>
        )}
        <div className="mt-6 flex gap-2">
          ...
          <button onClick={onCancel} className="rounded-lg bg-gray-200 px-4 py-2 text-sm dark:bg-gray-700">
            Cancel
          </button>
        </div>
      </div>
